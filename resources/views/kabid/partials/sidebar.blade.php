    ...
    <div class="p-6 flex-1 text-left">
        ...
        <nav class="space-y-1">
            ...
            <a href="{{ route('kabid.validasi') }}" 
               class="flex items-center gap-3 px-4 py-3 {{ request()->routeIs('kabid.validasi') ? 'bg-indigo-600 text-white font-bold shadow-lg shadow-indigo-900/20' : 'text-gray-400 hover:text-white hover:bg-white/5' }} rounded-xl text-sm font-semibold transition group text-left">
                <i class="fas fa-clipboard-check {{ request()->routeIs('kabid.validasi') ? '' : 'group-hover:text-indigo-400' }}"></i> 
                Validasi Data
            </a>

            <a href="{{ route('kabid.statistik.tahunan') }}" 
               class="flex items-center gap-3 px-4 py-3 {{ request()->routeIs('kabid.statistik.tahunan') ? 'bg-indigo-600 text-white font-bold shadow-lg shadow-indigo-900/20' : 'text-gray-400 hover:text-white hover:bg-white/5' }} rounded-xl text-sm font-semibold transition group text-left">
                <i class="fas fa-chart-line {{ request()->routeIs('kabid.statistik.tahunan') ? '' : 'group-hover:text-indigo-400' }}"></i> 
                Statistik Laporan
            </a>

            <a href="{{ route('kabid.laporan') }}" 
               class="flex items-center gap-3 px-4 py-3 {{ request()->routeIs('kabid.laporan') ? 'bg-indigo-600 text-white font-bold shadow-lg shadow-indigo-900/20' : 'text-gray-400 hover:text-white hover:bg-white/5' }} rounded-xl text-sm font-semibold transition group text-left">
                <i class="fas fa-print {{ request()->routeIs('kabid.laporan') ? '' : 'group-hover:text-indigo-400' }}"></i> 
                Cetak Laporan
            </a>
        </nav>
    </div>
