                    ...
                    <div class="space-y-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                            <div class="md:col-span-2">
                                <label class="block text-[10px] font-black text-[#1e1b4b] uppercase mb-2">Deskripsi Kondisi Lapangan</label>
                                <div class="w-full px-5 py-4 bg-gray-50 border border-gray-100 rounded-2xl text-sm font-semibold text-gray-600 italic relative overflow-hidden">
                                    @if(strtolower($inf->kondisi) == 'menunggu ai')
                                        <div class="flex items-center gap-2 text-amber-600 mb-1">
                                            <i class="fas fa-exclamation-triangle text-xs"></i>
                                            <span class="text-[9px] font-black uppercase tracking-widest">Peringatan: Deskripsi Belum Lengkap</span>
                                        </div>
                                        <p class="text-gray-400">"{{ $inf->kondisi }}"</p>
                                        <p class="mt-2 text-[10px] text-gray-400 not-italic font-medium">Silakan edit data ini dan masukkan kata kunci kerusakan (seperti: Retak, Lubang, atau Patah) agar sistem Decision Tree dapat memberikan skor prioritas yang akurat.</p>
                                    @else
                                        <p>"{{ $inf->kondisi ?? '-' }}"</p>
                                    @endif
                                </div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                                ...
                                <div class="bg-white/5 border border-white/10 rounded-3xl p-6 backdrop-blur-sm">
                                    ...
                                    <div class="flex items-end gap-3 mb-2">
                                        <p class="text-4xl font-black text-white">{{ $hasilCnn ? round($hasilCnn->skor_cnn * 100) : '0' }}<span class="text-sm font-bold text-indigo-300 ml-1">%</span></p>
                                        <p class="text-xs font-bold text-emerald-400 mb-2">{{ $hasilCnn->label_kondisi ?? 'Scanning...' }}</p>
                                        @if(!$hasilCnn)
                                            <form action="{{ route('admin.infrastruktur.analisis-ai', $inf->id_infrastruktur) }}" method="POST" class="mb-2">
                                                @csrf
                                    ...
                                    <div class="w-full bg-white/10 h-2 rounded-full overflow-hidden">
                                        <div class="bg-gradient-to-r from-indigo-500 to-emerald-400 h-full transition-all duration-1000" style="width: {{ $hasilCnn ? ($hasilCnn->skor_cnn * 100) : '0' }}%"></div>
                                    </div>
                                </div>
                                ...
                                <div class="bg-white/5 border border-white/10 rounded-3xl p-6 backdrop-blur-sm">
                                    ...
                                    <div class="flex items-end gap-3 mb-2">
                                        <p class="text-4xl font-black text-white">{{ $hasilAi->skor_dt ?? '0' }}<span class="text-sm font-bold text-indigo-300 ml-1">/100</span></p>
                                        <p class="text-xs font-bold {{ ($hasilAi->label_prioritas ?? '') == 'Rusak Berat' ? 'text-red-400' : 'text-yellow-400' }} mb-2">
                                            {{ $hasilAi->label_prioritas ?? 'Pending' }}
                                        </p>
                                    </div>
                                    <div class="w-full bg-white/10 h-2 rounded-full overflow-hidden">
                                        <div class="bg-gradient-to-r from-indigo-500 to-yellow-400 h-full transition-all duration-1000" style="width: {{ $hasilAi->skor_dt ?? '0' }}%"></div>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div class="space-y-6 pt-6 border-t border-gray-100">
                        ...
                        <div class="w-full max-w-2xl mx-auto relative rounded-2xl overflow-hidden border border-gray-100 shadow-sm bg-[#0f172a] aspect-video flex items-center justify-center group">
                            @if($inf->foto_terbaru && $inf->foto_terbaru != 'default.jpg')
                                ...
                                <img src="{{ asset('storage/' . (str_contains($cleanPath, 'infrastruktur/') ? $cleanPath : 'infrastruktur/' . $cleanPath)) }}" alt="Foto Infrastruktur" class="max-w-full max-h-full object-contain transition-transform duration-500 group-hover:scale-105">
                                
                                {{-- Simulasi Deteksi AI yang Lebih Dinamis --}}
                                @if(strtolower($inf->kondisi) != 'baik' && strtolower($inf->kondisi) != 'menunggu ai')
                                    <div class="absolute inset-0 flex items-center justify-center pointer-events-none">
                                        {{-- Kotak Deteksi dengan Sudut (Style Computer Vision) --}}
                                        <div class="relative w-1/2 h-1/2 border-2 border-red-500/50 bg-red-500/5 shadow-[0_0_20px_rgba(239,68,68,0.3)] animate-pulse">
                                            ...
                                            <div class="absolute -top-8 left-0 bg-red-600 text-white text-[9px] font-black px-3 py-1 rounded-md shadow-lg flex items-center gap-2">
                                                ...
                                                <span>AREA KERUSAKAN TERDETEKSI ({{ round(($hasilCnn->skor_cnn ?? 0) * 100) }}%)</span>
                                            </div>
                                ...
                                <div class="absolute inset-0 bg-black/40 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                                    <a href="{{ asset('storage/' . (str_contains($cleanPath, 'infrastruktur/') ? $cleanPath : 'infrastruktur/' . $cleanPath)) }}" target="_blank" class="bg-white text-[#1e1b4b] px-6 py-3 rounded-2xl text-xs font-black shadow-2xl uppercase tracking-widest hover:scale-110 transition-all flex items-center gap-2">
