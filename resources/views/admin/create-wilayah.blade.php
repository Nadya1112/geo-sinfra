...
<body class="bg-navy-50 dark:bg-navy-950 text-slate-800 dark:text-slate-200 dark:text-slate-200 antialiased flex overflow-hidden h-screen transition-colors duration-300">
    ...
    <main class="flex-1 flex flex-col h-screen overflow-hidden text-left font-sans">
        <header class="bg-white/85 dark:bg-navy-950/80 backdrop-blur-xl border-b border-slate-100 dark:border-white/10 dark:border-white/5 px-4 md:px-8 py-4 flex justify-between items-center z-40 text-left transition-colors duration-300">
            <div class="flex items-center gap-4 text-left">
                ...
                <div class="text-left">
                    ...
                    <h2 class="text-xl font-black text-navy-900 dark:text-white dark:text-white leading-none">Tambah Data Wilayah</h2>
                </div>
            </div>
        ...
        <div class="flex-1 p-8 overflow-y-auto custom-scrollbar text-left">
            ...
            <div class="max-w-3xl bg-white dark:bg-navy-900/90 dark:backdrop-blur-xl rounded-[2.5rem] p-10 border border-slate-100 dark:border-white/10 shadow-sm mx-auto text-left">
                <form action="{{ route('admin.wilayah.store') }}" method="POST">
                    @csrf
                    
                    <div class="space-y-6">
                        <div>
                            <label class="block text-xs font-black text-navy-900 dark:text-white uppercase tracking-widest mb-2">Pilih Kecamatan <span class="text-red-500">*</span></label>
                            <div class="relative">
                                <select name="id_kecamatan" class="w-full px-5 py-3 bg-slate-50 dark:bg-navy-950/50 border border-slate-100 dark:border-white/10 rounded-2xl text-sm font-semibold focus:ring-4 focus:ring-gold-500/10 focus:border-gold-500 outline-none appearance-none" required>
                        ...
                        <div>
                            <label class="block text-xs font-black text-navy-900 dark:text-white uppercase tracking-widest mb-2">Nama Kelurahan <span class="text-red-500">*</span></label>
                            <input type="text" name="nama_kelurahan" value="{{ old('nama_kelurahan') }}" placeholder="Contoh: Antasan Besar" class="w-full px-5 py-3 bg-slate-50 dark:bg-navy-950/50 border border-slate-100 dark:border-white/10 rounded-2xl text-sm font-semibold focus:ring-4 focus:ring-gold-500/10 focus:border-gold-500 outline-none transition-all" required>
                        </div>
                        ...
                        <div class="pt-8 flex gap-3">
                            ...
                            <a href="{{ route('admin.wilayah') }}" class="flex-1 bg-slate-100 dark:bg-navy-950/50 text-slate-500 dark:text-slate-400 text-xs px-4 py-3 rounded-2xl font-bold hover:bg-slate-200 transition text-center flex items-center justify-center gap-2 tracking-widest uppercase">
                                Batal
                            </a>
                        </div>
                    </div>
