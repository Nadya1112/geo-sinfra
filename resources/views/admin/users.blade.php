                        ...
                        <tr class="hover:bg-slate-50/50 transition">
                            ...
                            <td class="px-8 py-5">
                                <div class="flex items-center gap-4">
                                    <div class="w-10 h-10 {{ $user->role == 'admin' ? 'bg-gold-500/10 text-gold-500 border-gold-500/20' : ($user->role == 'kabid' ? 'bg-navy-900/10 text-navy-900 border-navy-900/20' : 'bg-slate-100 text-slate-600 border-slate-200') }} rounded-xl flex items-center justify-center font-bold text-xs border">
                                        {{ substr($user->name, 0, 1) }}
                                    </div>
                                    <div>
                                        <p class="text-xs font-black text-navy-900 uppercase leading-none">{{ $user->name }}</p>
                                        <p class="text-[9px] text-slate-400 font-bold uppercase mt-1 italic">ID: #{{ str_pad($user->id, 4, '0', STR_PAD_LEFT) }}</p>
                                    </div>
                                </div>
                            </td>
                            <td class="px-8 py-5 text-[11px] font-medium text-slate-500">{{ $user->email }}</td>
                            <td class="px-8 py-5">
                                <span class="px-4 py-1.5 rounded-full text-[9px] font-black uppercase tracking-tighter 
                                    {{ $user->role == 'admin' ? 'bg-gold-500/10 text-gold-500 border border-gold-500/20' : ($user->role == 'kabid' ? 'bg-navy-900/10 text-navy-900 border border-navy-900/20' : 'bg-slate-100 text-slate-600 border border-slate-200') }}">
                                    {{ $user->role }}
                                </span>
                            </td>
                            <td class="px-8 py-5">
