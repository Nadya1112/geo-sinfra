...
<header class="sticky top-0 bg-white/80 dark:bg-navy-950/80 backdrop-blur-xl border-b border-slate-100 dark:border-white/5 px-4 pl-16 md:px-8 py-4 flex justify-between items-center z-40 text-left transition-colors duration-300 shadow-sm">
    ...
    <div class="flex items-center gap-3 md:gap-6">
        <div class="text-right">
            ...
            <p class="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-tighter hidden sm:block">{{ now()->translatedFormat('l, d F Y') }}</p>
        </div>
        ...
        <div class="flex items-center gap-3">
            ...
            <div class="relative group/theme">
                <button type="button" onclick="document.getElementById('theme-dropdown-header').classList.toggle('hidden')" class="w-10 h-10 bg-white dark:bg-navy-900 rounded-xl flex items-center justify-center text-slate-400 hover:text-gold-500 hover:bg-gold-50 dark:hover:bg-white/5 border border-slate-200 dark:border-white/10 transition-all shadow-sm relative z-[6000] cursor-pointer" title="Ganti Tema">
                    <i class="fas fa-sun theme-toggle-sun pointer-events-none text-lg"></i>
                    <i class="fas fa-moon theme-toggle-moon pointer-events-none text-lg"></i>
                </button>
                <!-- Dropdown Content -->
