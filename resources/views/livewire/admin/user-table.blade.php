<div>
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
        ...
        <div class="flex flex-col md:flex-row items-center gap-2 w-full md:w-auto">
            <div class="flex flex-col sm:flex-row items-stretch sm:items-center w-full md:w-[400px]">
                <select wire:model.live="show" class="w-full sm:w-auto pl-3 pr-7 py-2.5 bg-white dark:bg-navy-900/90 dark:backdrop-blur-xl border border-slate-100 dark:border-white/10 sm:border-r-0 rounded-t-xl sm:rounded-t-none sm:rounded-l-2xl text-[10px] md:text-xs font-bold text-navy-900 dark:text-white focus:outline-none focus:ring-4 focus:ring-gold-500/10 focus:border-gold-500 transition-all shadow-sm shrink-0">
    ...
    <div class="bg-white dark:bg-navy-900/90 dark:backdrop-blur-xl rounded-3xl md:rounded-[2.5rem] border border-slate-100 dark:border-white/10 shadow-sm overflow-hidden mb-10 relative">
...
<div class="overflow-x-auto w-full custom-scrollbar" wire:loading.class="opacity-50 transition-opacity duration-300" wire:target="search, show, filter, setFilter">
            ...
            <thead>
                <tr class="bg-gradient-to-r from-gold-500 to-gold-600 dark:from-navy-900 dark:to-navy-800 border-b border-gold-600 dark:border-navy-800 shadow-sm dark:shadow-md">
                    <th class="px-4 md:px-4 py-3 text-xs font-black text-white dark:text-gold-500 uppercase tracking-widest w-12 text-center">No.</th>
                    <th class="px-4 md:px-4 py-3 text-xs font-black text-white dark:text-gold-500 uppercase tracking-widest">Nama User</th>
                    <th class="px-4 md:px-4 py-3 text-xs font-black text-white dark:text-gold-500 uppercase tracking-widest">Alamat Email</th>
                    <th class="px-4 md:px-4 py-3 text-xs font-black text-white dark:text-gold-500 uppercase tracking-widest">Role / Jabatan</th>
                    <th class="px-4 md:px-4 py-3 text-xs font-black text-white dark:text-gold-500 uppercase tracking-widest text-center">Aksi</th>
                </tr>
            </thead>
            <tbody class="divide-y divide-black dark:divide-white/5">
                ...
                <tr class="hover:bg-slate-50/50 dark:hover:bg-white/5 transition">
                    <td class="px-4 md:px-4 py-3 text-center">
                        <span class="text-xs font-black text-navy-900 dark:text-white">{{ $show == 'all' ? $index + 1 : ($users->currentPage() - 1) * $users->perPage() + $index + 1 }}</span>
                    </td>
                ...
                @empty
                <tr>
                    <td colspan="5" class="px-4 py-8 text-center text-slate-500 dark:text-slate-400 text-xs font-medium">
            ...
            <div class="flex flex-col md:hidden divide-y divide-black dark:divide-white/10">
                ...
                <div class="p-4 hover:bg-slate-50 dark:hover:bg-white/5 dark:bg-navy-950/50 transition">
                    <div class="flex items-start gap-4">
                        ...
                        <div class="flex-1 min-w-0">
                            <h5 class="text-sm font-black text-navy-900 dark:text-white uppercase leading-none truncate mb-1">{{ $user->name }}</h5>
                ...
                @empty
                <div class="p-8 text-center text-slate-500 dark:text-slate-400 text-xs font-medium">
                    Tidak ada data pengguna yang ditemukan.
                </div>
                @endforelse
            </div>
