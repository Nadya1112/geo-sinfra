        ...
        <div class="max-w-7xl mx-auto px-6 md:px-8">
            ...
            <div class="grid grid-cols-2 lg:grid-cols-4 gap-6 reveal-up">
                ...
                <div class="bg-slate-50 p-8 rounded-3xl border border-slate-100 shadow-sm hover:shadow-md hover:bg-white transition-all duration-300 text-center relative group">
                    ...
                    <p class="text-4xl md:text-5xl font-black text-navy-900 leading-none mb-2">{{ number_format($stats['total'] ?? 0) }}</p>
                ...
                <div class="bg-slate-50 p-8 rounded-3xl border border-slate-100 shadow-sm hover:shadow-md hover:bg-white transition-all duration-300 text-center relative group">
                    ...
                    <p class="text-4xl md:text-5xl font-black text-navy-900 leading-none mb-2">{{ number_format($stats['kecamatan'] ?? 0) }}</p>
                ...
                <div class="bg-slate-50 p-8 rounded-3xl border border-slate-100 shadow-sm hover:shadow-md hover:bg-white transition-all duration-300 text-center relative group">
                    ...
                    <p class="text-4xl md:text-5xl font-black text-navy-900 leading-none mb-2">{{ number_format($stats['rusak_berat'] ?? 0) }}</p>
                        ...
                        @foreach($sebaranKecamatan as $nama => $count)
                            <div>
                                <div class="flex justify-between text-xs font-bold uppercase tracking-wider mb-2">
                                    <span class="text-slate-500">{{ $nama ?: 'Wilayah Tidak Diketahui' }}</span>
                                    <span class="text-navy-900 font-extrabold">{{ $count }} Titik</span>
                                </div>
                                <div class="w-full bg-slate-100 h-3 rounded-full overflow-hidden">
                                    <div class="bg-gradient-to-r from-navy-800 to-gold-500 h-full rounded-full" style="width: {{ $stats['total'] > 0 ? ($count / $stats['total'] * 100) : 0 }}%"></div>
                                </div>
                            </div>
                        @endforeach
                    </div>
                    ...
                    <div class="relative w-full z-40">
                        ...
                        <div id="wilayah-menu" class="hidden absolute top-full mt-2 w-full bg-[#0f0e2c]/95 backdrop-blur-2xl rounded-2xl p-2.5 shadow-2xl border border-white/10 max-h-[300px] overflow-y-auto custom-scrollbar">
                            ...
                            <label class="flex items-center justify-between p-3 hover:bg-white/5 rounded-xl cursor-pointer transition-all group">
                                <div class="flex items-center gap-3">
                                    <input type="checkbox" class="filter-district w-4.5 h-4.5 rounded border-slate-600 bg-transparent text-gold-500 focus:ring-0" value="{{ $wil->id_kecamatan }}" checked>
                                    <span class="text-[10px] font-bold text-slate-200 uppercase tracking-wider">{{ $wil->nama_kecamatan }}</span>
                                </div>
                                <div class="w-2.5 h-2.5 rounded-full" style="background: {{ $kecColors[$index % count($kecColors)] }}"></div>
                            </label>
                            @endforeach
                        </div>
                    </div>
