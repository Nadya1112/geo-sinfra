        ...
        <header class="bg-white/85 backdrop-blur-xl border-b border-slate-100 sticky top-0 px-4 pl-20 md:px-8 py-4 flex justify-between items-center z-40">
            <div class="flex items-center gap-4">
                ...
                <div class="text-left">
                    ...
                    <h2 class="text-xl font-black text-navy-900 leading-none">Informasi Laporan Warga</h2>
                </div>
            </div>
        </header>
            ...
            <div class="max-w-4xl mx-auto space-y-6">
                <div class="bg-white rounded-[2rem] p-6 md:p-8 shadow-sm border border-slate-100">
                    <div class="flex flex-col md:flex-row justify-between items-start gap-4 mb-6">
                        <div>
                            <h3 class="text-lg font-black text-navy-900">Deskripsi Laporan</h3>
                        ...
                        <div class="px-4 py-2 bg-slate-50 rounded-xl border border-slate-200">
                            <span class="text-xs font-black text-navy-900 uppercase tracking-widest">Status: </span>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
                        <div>
                            <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5">Nama Pelapor</p>
                            <p class="text-sm font-bold text-navy-900">{{ $laporan->nama_pelapor }}</p>
                        </div>
                        <div>
                            <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5">No HP / WhatsApp</p>
                            <p class="text-sm font-bold text-navy-900"><i class="fab fa-whatsapp text-emerald-500 mr-1"></i>{{ $laporan->no_hp }}</p>
                        </div>
                    </div>

                    <div>
                        <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Isi Laporan Kerusakan</p>
                ...
                <div class="bg-white rounded-[2rem] p-6 md:p-8 shadow-sm border border-slate-100">
                    <h3 class="text-lg font-black text-navy-900 mb-6">Lokasi & Lampiran Foto</h3>
                    
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        <div>
                            <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Titik Koordinat</p>
                            <div class="flex items-center justify-between p-4 bg-slate-50 rounded-xl border border-slate-100 mb-4">
                                <div class="flex items-center gap-3">
                                    <div class="w-10 h-10 bg-emerald-100 text-emerald-600 rounded-lg flex items-center justify-center">
                                        <i class="fas fa-map-marker-alt"></i>
                                    </div>
                                    <div>
                                        <p class="text-xs font-black text-navy-900">{{ $laporan->latitude }}, {{ $laporan->longitude }}</p>
                                        <p class="text-[10px] text-slate-500 font-bold uppercase mt-0.5">Koordinat Google Maps</p>
                                    </div>
                                </div>
                                <a href="https://www.google.com/maps/search/?api=1&query={{ $laporan->latitude }},{{ $laporan->longitude }}" target="_blank" class="px-4 py-2 bg-white border border-slate-200 rounded-lg text-xs font-bold text-navy-900 hover:bg-emerald-50 hover:text-emerald-600 hover:border-emerald-200 transition-all shadow-sm">
                                    Buka di Maps
                                </a>
                            </div>
                        </div>
                        ...
                        <div>
                            <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Foto Laporan Warga</p>
                            @if($laporan->foto)
                                <a href="{{ asset('storage/' . $laporan->foto) }}" target="_blank" class="block w-full h-40 rounded-xl overflow-hidden group relative border border-slate-200">
                                    ...
                                    <div class="absolute inset-0 bg-navy-900/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                                        <div class="bg-white/90 backdrop-blur-sm px-4 py-2 rounded-lg text-xs font-bold text-navy-900 flex items-center gap-2">
                                            <i class="fas fa-expand"></i> Lihat Penuh
                                        </div>
                                    </div>
                                </a>
                            @else
                                <div class="w-full h-40 rounded-xl bg-slate-50 border border-slate-200 flex flex-col items-center justify-center text-slate-400">
