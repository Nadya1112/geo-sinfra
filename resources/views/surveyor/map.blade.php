...
<body class="bg-gray-50 flex h-screen overflow-hidden text-gray-800 text-left">
        ...
        <div class="flex-1 relative">
            ...
            <div class="absolute top-6 left-6 z-10 space-y-4 max-w-xs">
                ...
                <div class="bg-[#1e1b4b]/90 backdrop-blur-md p-6 rounded-[2rem] border border-white/10 shadow-xl text-white">
                    ...
                    <h5 class="text-lg font-black">{{ $dataMap->count() }} <span class="text-xs font-medium text-blue-300">Titik Laporan</span></h5>
                </div>
            </div>
        </div>
    </main>
    ...
    <script>
        ...
        if (dataPoints.length > 0) {
            const group = new L.featureGroup(dataPoints.map(p => L.marker([p.latitude, p.longitude])));
