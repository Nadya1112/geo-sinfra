    ...
    <main class="flex-1 flex flex-col h-screen overflow-hidden text-left relative">
        <header class="bg-white/85 backdrop-blur-xl border-b border-slate-100 dark:border-white/10 sticky top-0 px-4 pl-20 md:px-8 py-4 flex justify-between items-center z-40">
            <div class="flex items-center gap-4">
                ...
                <div class="text-left">
                    ...
                    <h2 class="text-xl font-black text-navy-900 dark:text-white leading-none">Ubah Status Laporan Warga</h2>
                </div>
            </div>
        </header>
        ...
        <div class="flex-1 overflow-y-auto custom-scrollbar p-6 md:p-8">
            <div class="max-w-4xl mx-auto">
                <div class="bg-white dark:bg-navy-900/90 rounded-[2rem] p-6 md:p-8 shadow-sm border border-slate-100 dark:border-white/10 mb-6">
                    <form id="editForm" action="{{ route('surveyor.laporan.update', $laporan->id) }}" method="POST">
                        ...
                        <div class="mb-8">
                            <label class="block text-[10px] font-black text-slate-400 dark:text-slate-300 uppercase tracking-widest mb-2">Pilih Status Laporan</label>
                            <div class="relative">
                                <select id="statusSelect" name="status" class="w-full bg-slate-50 dark:bg-[#0b0a26]/50 border border-slate-200 dark:border-white/10 rounded-xl px-4 py-3.5 text-sm font-bold text-navy-900 dark:text-white focus:outline-none focus:ring-4 focus:ring-gold-500/10 focus:border-gold-500 transition-all appearance-none cursor-pointer">
