...
<body class="bg-slate-50  flex h-screen overflow-hidden text-slate-800 font-sans   transition-colors duration-300">
    ...
    <main class="flex-1 flex flex-col h-screen overflow-hidden">
        ...
        <header class="bg-white/80  backdrop-blur-xl border-b border-slate-100  sticky top-0 px-4  md:px-8 py-4 flex justify-between items-center z-40 shrink-0 shadow-sm">
            <div class="flex items-center gap-4">
                <a href="<?php echo e(route('surveyor.history')); ?>" class="hidden md:flex w-10 h-10  items-center justify-center bg-white  text-slate-400 rounded-xl hover:bg-gold-50 hover:text-gold-600 transition-all border border-slate-200  hover:border-gold-200">
            ...
            <div class="flex items-center gap-6">
                ...
                <a href="<?php echo e(route('surveyor.profile')); ?>" class="flex items-center gap-3">
                    <div class="text-right">
                        <p class="text-xs font-black text-navy-900  leading-none uppercase max-w-[100px] sm:max-w-[150px] md:max-w-[300px] truncate"><?php echo e(auth()->user()->name); ?></p>
        ...
        <div class="flex-1 overflow-y-auto custom-scrollbar p-4 md:p-8 pb-16">
            <div class="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div class="bg-white  rounded-[2.5rem] p-4 border border-slate-100  shadow-sm overflow-hidden hover:shadow-md transition-shadow">
                        <div class="relative aspect-[3/4] w-full rounded-[2rem] overflow-hidden bg-navy-950 group flex items-center justify-center">
                                ...
                                <div class="absolute inset-0 flex items-center justify-center pointer-events-none p-6">
                                    <div class="relative w-[50%] h-[50%] border-2 border-red-500/50 bg-red-500/5 animate-pulse">
                                        ...
                                        <div class="absolute -top-6 left-0 bg-red-500 text-white text-xs font-black px-2 py-0.5 rounded-md shadow-lg tracking-widest">
                                            KERUSAKAN TERDETEKSI (<?php echo e(round(($hasilCnn->skor_cnn ?? 0) * 100)); ?>%)
                                        </div>
                                    </div>
                                </div>
                            <?php endif; ?>
                            ...
                            <div class="absolute inset-0 bg-navy-900/60 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all backdrop-blur-sm">
                                <a href="<?php echo e(asset('storage/' . (str_contains($cleanPath, 'infrastruktur/') ? $cleanPath : 'infrastruktur/' . $cleanPath))); ?>" target="_blank" class="bg-gold-500 text-white px-5 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest hover:bg-gold-600 hover:scale-105 transition-all shadow-xl">
                        ...
                        <div class="space-y-8 relative z-10">
                            ...
                            <div class="relative">
                                <div class="flex justify-between items-end mb-2">
                                    ...
                                    <p class="text-xl font-black text-gold-500"><?php echo e($hasilCnn ? round($hasilCnn->skor_cnn * 100) : '0'); ?>%</p>
                                </div>
                                <div class="w-full bg-white/5  h-2 rounded-full overflow-hidden border border-white/5">
                                    <div class="bg-gold-500 h-full shadow-[0_0_10px_rgba(197,160,89,0.5)]" style="width: <?php echo e($hasilCnn ? ($hasilCnn->skor_cnn * 100) : '0'); ?>%"></div>
                                </div>
                                <p class="text-xs font-bold text-slate-400 mt-2 italic text-right"><?php echo e($hasilCnn->label_kondisi ?? 'Memeriksa visual lapangan...'); ?></p>
                            </div>
                            ...
                            <div class="relative">
                                <div class="flex justify-between items-end mb-2">
                                    ...
                                    <p class="text-xl font-black text-white"><?php echo e($infrastruktur->analisis->skor_dt ?? '0'); ?><span class="text-xs text-slate-500 ml-0.5">/100</span></p>
                                </div>
                                <div class="w-full bg-white/5  h-2 rounded-full overflow-hidden border border-white/5">
                                    <?php $dtColor = ($infrastruktur->analisis->label_prioritas ?? '') == 'Rusak Berat' ? 'bg-red-500' : (($infrastruktur->analisis->label_prioritas ?? '') == 'Rusak Sedang' ? 'bg-amber-500' : 'bg-emerald-500'); ?>
                                    <div class="<?php echo e($dtColor); ?> h-full" style="width: <?php echo e($infrastruktur->analisis->skor_dt ?? '0'); ?>%"></div>
                                </div>
                                <p class="text-xs font-bold mt-2 italic text-right <?php echo e(($infrastruktur->analisis->label_prioritas ?? '') == 'Rusak Berat' ? 'text-red-400' : (($infrastruktur->analisis->label_prioritas ?? '') == 'Rusak Sedang' ? 'text-amber-400' : 'text-emerald-400')); ?>">
                                    Label: <?php echo e($infrastruktur->analisis->label_prioritas ?? 'Menunggu Status...'); ?>
                            ...
                            <div class="pt-4 border-t border-white/10 space-y-2">
                                ...
                                <p class="text-sm font-medium text-slate-300 leading-relaxed bg-white/5  p-4 rounded-2xl border border-white/5">
                                    <?php echo e($infrastruktur->analisis->rekomendasi ?? 'Tindakan rekomendasi belum tersedia.'); ?>
                ...
                <?php if($infrastruktur->alasan_penolakan): ?>
                ...
                <div class="lg:col-span-2 space-y-6">
                    <div class="bg-white  rounded-[2.5rem] p-8 lg:p-10 border border-slate-100  shadow-sm hover:shadow-md transition-shadow space-y-8">
                        ...
                        <div class="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-4 border-b border-slate-50 pb-6">
                            <div>
                                <div class="inline-flex items-center gap-2 px-3 py-1 bg-navy-50  border border-navy-100 rounded-lg mb-3">
                                    ...
                                    <p class="text-xs font-black text-navy-900  uppercase tracking-widest"><?php echo e(ucfirst($infrastruktur->jenis)); ?></p>
                                </div>
                            ...
                            <div class="sm:text-right shrink-0">
                                ...
                                <p class="text-xs font-black text-navy-900  mt-1"><?php echo e($infrastruktur->created_at->translatedFormat('d M Y, H:i')); ?></p>
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                            <div class="bg-slate-50  p-5 rounded-[1.5rem] border border-slate-200  flex items-center gap-4 hover:border-gold-300 transition-colors">
                                ...
                                <div>
                                    ...
                                    <p class="text-sm font-black text-navy-900  mt-0.5"><?php echo e($infrastruktur->kelurahan->kecamatan->nama_kecamatan ?? '-'); ?></p>
                                </div>
                            </div>
                            <div class="bg-slate-50  p-5 rounded-[1.5rem] border border-slate-200  flex items-center gap-4 hover:border-gold-300 transition-colors">
                                <div class="w-12 h-12 rounded-2xl bg-white  flex items-center justify-center text-gold-500 shadow-sm">
                                ...
                                <div>
                                    ...
                                    <p class="text-sm font-black text-navy-900  mt-0.5"><?php echo e($infrastruktur->kelurahan->nama_kelurahan ?? '-'); ?></p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="pt-2">
                            <h4 class="text-sm font-black text-navy-900  uppercase tracking-widest mb-4 flex items-center gap-2">
                            ...
                            <div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
                                <div class="bg-white  p-4 rounded-[1.5rem] border border-slate-200  shadow-sm text-center">
                                    ...
                                    <span class="text-xs font-black text-navy-900 "><?php echo e($infrastruktur->panjang ?? '0'); ?> m</span>
                                </div>
                                <div class="bg-white  p-4 rounded-[1.5rem] border border-slate-200  shadow-sm text-center">
                                    <span class="block text-xs font-black text-slate-400 uppercase tracking-widest mb-1.5">Lebar</span>
                                    <span class="text-xs font-black text-navy-900 "><?php echo e($infrastruktur->lebar ?? '0'); ?> m</span>
                                </div>
                                <div class="bg-white  p-4 rounded-[1.5rem] border border-slate-200  shadow-sm text-center">
                                    <span class="block text-xs font-black text-slate-400 uppercase tracking-widest mb-1.5">Material</span>
                                    <span class="text-sm font-black text-navy-900 "><?php echo e($infrastruktur->material_eksisting ?? '-'); ?></span>
                                </div>
                                <div class="bg-white  p-4 rounded-[1.5rem] border border-slate-200  shadow-sm text-center">
                                    <span class="block text-xs font-black text-slate-400 uppercase tracking-widest mb-1.5">Drainase</span>
                                    <?php if($infrastruktur->has_drainase == 'ya'): ?>
                                        <span class="inline-flex px-2 py-0.5 bg-emerald-50 text-emerald-600 rounded text-xs font-black uppercase">Tersedia</span>
                                    <?php else: ?>
                                        <span class="inline-flex px-2 py-0.5 bg-slate-50  text-slate-500 rounded text-xs font-black uppercase">Tidak</span>
                                    <?php endif; ?>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="pt-2">
                            ...
                            <p class="text-xs font-black text-slate-400 text-center tracking-widest mt-3">
                                LAT: <span class="text-navy-900 "><?php echo e($infrastruktur->latitude); ?></span> &nbsp;|&nbsp; LNG: <span class="text-navy-900 "><?php echo e($infrastruktur->longitude); ?></span>
                            </p>
                        </div>
...
<?php /**PATH C:\laragon1\laragon\www\geo-sinfra\resources\views\surveyor\show.blade.php ENDPATH**/ ?>