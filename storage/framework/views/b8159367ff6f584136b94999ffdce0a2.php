    ...
    <main class="flex-1 flex flex-col h-screen overflow-hidden text-left font-sans">
        <header class="bg-white/85 backdrop-blur-xl border-b border-slate-100 px-8 py-5 flex justify-between items-center z-40 text-left">
            <div class="flex items-center gap-4 text-left">
                ...
                <div class="text-left">
                    ...
                    <h2 class="text-xl font-black text-navy-900 leading-none">Edit Data Pengguna</h2>
                </div>
            </div>
        ...
        <div class="flex-1 p-8 overflow-y-auto custom-scrollbar">
            <div class="max-w-4xl bg-white rounded-[2.5rem] p-10 border border-slate-100 shadow-sm mx-auto text-left">
                <form action="<?php echo e(route('admin.users.update', $user->id)); ?>" method="POST" class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <?php echo csrf_field(); ?>
                    <?php echo method_field('PUT'); ?>
                    
                    <div class="space-y-6">
                        <div>
                            <label class="block text-[10px] font-black text-navy-900 uppercase tracking-widest mb-2">Nama Pengguna <span class="text-red-500">*</span></label>
                            <input type="text" name="name" value="<?php echo e($user->name); ?>" class="w-full px-5 py-3 bg-slate-50 border border-slate-100 rounded-2xl text-sm font-semibold focus:ring-4 focus:ring-gold-500/10 focus:border-gold-500 outline-none transition-all" required>
                        </div>

                        <div>
                            <label class="block text-[10px] font-black text-navy-900 uppercase tracking-widest mb-2">Email <span class="text-red-500">*</span></label>
                            <input type="email" name="email" value="<?php echo e($user->email); ?>" class="w-full px-5 py-3 bg-slate-50 border border-slate-100 rounded-2xl text-sm font-semibold focus:ring-4 focus:ring-gold-500/10 focus:border-gold-500 outline-none transition-all" required>
                        </div>

                        <div>
                            <label class="block text-[10px] font-black text-navy-900 uppercase tracking-widest mb-2">Nomor WhatsApp / HP <span class="text-red-500">*</span></label>
                            <input type="text" name="no_hp" value="<?php echo e($user->no_hp); ?>" class="w-full px-5 py-3 bg-slate-50 border border-slate-100 rounded-2xl text-sm font-semibold focus:ring-4 focus:ring-gold-500/10 focus:border-gold-500 outline-none transition-all" required>
                        </div>

                        <div>
                            <label class="block text-[10px] font-black text-navy-900 uppercase tracking-widest mb-2">Ganti Password (Kosongkan jika tetap)</label>
                            <div class="relative">
                    ...
                    <div class="space-y-6">
                        <div>
                            <label class="block text-[10px] font-black text-navy-900 uppercase tracking-widest mb-2">Role Akses <span class="text-red-500">*</span></label>
                            <select id="role-select" name="role" class="w-full px-5 py-3 bg-slate-50 border border-slate-100 rounded-2xl text-sm font-semibold focus:ring-4 focus:ring-gold-500/10 focus:border-gold-500 outline-none transition-all" onchange="toggleWilayah()">
                                ...
                                <option value="surveyor" <?php echo e($user->role == 'surveyor' ? 'selected' : ''); ?>>SURVEYOR</option>
                            </select>
                        </div>

                        <div id="wilayah-container" class="<?php echo e($user->role == 'admin' ? 'hidden' : ''); ?>">
                            ...
                            <select name="id_kecamatan" class="w-full px-5 py-3 bg-slate-50 border border-slate-100 rounded-2xl text-sm font-semibold focus:ring-4 focus:ring-gold-500/10 focus:border-gold-500 outline-none transition-all">
                                ...
                                <?php $__currentLoopData = $semuaWilayah; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $wilayah): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <option value="<?php echo e($wilayah->id_kecamatan); ?>" <?php echo e($user->id_kecamatan == $wilayah->id_kecamatan ? 'selected' : ''); ?>>
                                        Kec. <?php echo e($wilayah->nama_kecamatan); ?>
                        ...
                        <div class="pt-10 flex gap-3">
                            ...
                            <a href="<?php echo e(route('admin.users')); ?>" class="flex-1 bg-slate-100 text-slate-500 text-xs px-6 py-4 rounded-2xl font-bold hover:bg-slate-200 transition text-center flex items-center justify-center gap-2 tracking-widest uppercase">
...
</html><?php /**PATH C:\laragon1\laragon\www\geo-sinfra\resources\views/admin/edit-user.blade.php ENDPATH**/ ?>