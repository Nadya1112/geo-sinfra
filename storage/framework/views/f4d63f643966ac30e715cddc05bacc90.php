...
<body class="bg-slate-50 flex h-screen overflow-hidden text-slate-800 font-sans">
    ...
    <main class="flex-1 flex flex-col h-screen overflow-hidden">
        <header class="bg-white/80 backdrop-blur-xl border-b border-slate-100 px-8 py-5 flex justify-between items-center z-40 shrink-0">
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <h2 class="text-xl font-black text-navy-900 leading-none">Detail Laporan Warga</h2>
                </div>
            </div>
        ...
        <div class="flex-1 overflow-y-auto custom-scrollbar p-8 pb-16">
            <div class="flex items-center gap-3 mb-6">
                <span class="px-3 py-1.5 bg-emerald-500 text-white rounded-xl text-[9px] font-black tracking-widest uppercase">
                    <i class="fas fa-exchange-alt mr-1"></i> Konversi ke Aset Infrastruktur
                </span>
                <span class="text-[10px] text-slate-400 font-semibold">Lengkapi data teknis yang kurang. Foto dan kordinat akan otomatis disalin.</span>
            </div>
            ...
            <form action="<?php echo e(route('admin.laporan-warga.convert.store', $laporan->id)); ?>" method="POST">
                <?php echo csrf_field(); ?>
                    ...
                    <div class="xl:col-span-2 space-y-6">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                                <div class="md:col-span-2">
                                    ...
                                    <input type="text" name="nama_infrastruktur"
                                           value="<?php echo e(old('nama_infrastruktur') ?? Str::limit('Laporan: ' . $laporan->deskripsi, 50)); ?>"
                                ...
                                <div>
                                    ...
                                    <select name="jenis" class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-semibold text-slate-700 outline-none focus:ring-2 focus:ring-gold-500/20 focus:border-gold-500 transition-all" required>
                                        <?php $jenisTerpilih = old('jenis') ?? $laporan->jenis_ai; ?>
                                        <option value="" disabled <?php echo e(empty($jenisTerpilih) ? 'selected' : ''); ?>>-- Pilih Jenis --</option>
                                        <option value="jalan" <?php echo e($jenisTerpilih == 'jalan' ? 'selected' : ''); ?>>Jalan</option>
                                        <option value="titian" <?php echo e($jenisTerpilih == 'titian' ? 'selected' : ''); ?>>Titian</option>
                                        <option value="jembatan" <?php echo e($jenisTerpilih == 'jembatan' ? 'selected' : ''); ?>>Jembatan</option>
                                    </select>
                                </div>
                                <div>
                                ...
                                <div class="md:col-span-2">
                                    ...
                                    <select name="id_kelurahan" class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-semibold text-slate-700 outline-none focus:ring-2 focus:ring-gold-500/20 focus:border-gold-500 transition-all" required>
                                        <?php $__currentLoopData = $semuaKelurahan; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $kel): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                            <option value="<?php echo e($kel->id_kelurahan); ?>"><?php echo e($kel->nama_kelurahan); ?></option>
                                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                                    </select>
                                </div>
                            </div>
                        ...
                        <div class="bg-white rounded-3xl border border-slate-100 shadow-sm p-8">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-5 mb-5">
                                <div>
                                    <label class="block text-[10px] font-black text-navy-900 uppercase tracking-widest mb-2">Material <span class="text-red-400">*</span></label>
                            ...
                            <div>
                                ...
                                <textarea name="kondisi" rows="3" class="w-full px-4 py-3 bg-amber-50 border border-amber-100 rounded-xl text-sm font-semibold text-slate-700 outline-none focus:ring-2 focus:ring-gold-500/20 focus:border-gold-500 transition-all" required><?php echo e(old('kondisi') ?? $laporan->deskripsi); ?></textarea>
                            </div>
                        </div>
                        ...
                        <div class="bg-white rounded-3xl border border-slate-100 shadow-sm p-8">
                            ...
                            <div class="grid grid-cols-2 gap-5 mb-5">
                                <div>
                                    ...
                                    <input type="text" name="latitude" id="lat-input" value="<?php echo e($laporan->latitude); ?>" readonly class="w-full px-4 py-3 bg-slate-100 border border-slate-200 rounded-xl text-sm font-semibold text-slate-500">
                                </div>
                                <div>
                                    <label class="block text-[10px] font-black text-navy-900 uppercase tracking-widest mb-2">Longitude</label>
                                    <input type="text" name="longitude" id="lng-input" value="<?php echo e($laporan->longitude); ?>" readonly class="w-full px-4 py-3 bg-slate-100 border border-slate-200 rounded-xl text-sm font-semibold text-slate-500">
                                </div>
                            </div>
                    ...
                    <div class="space-y-6">
                        <div class="bg-white rounded-3xl border border-slate-100 shadow-sm p-6">
                            ...
                            <div class="relative w-full h-48 rounded-2xl border border-slate-200 bg-slate-50 overflow-hidden">
                                <?php if($laporan->foto): ?>
                                    <img src="<?php echo e(asset('storage/' . $laporan->foto)); ?>" class="w-full h-full object-cover" alt="Foto Laporan">
                                <?php else: ?>
                        ...
                        <div class="bg-white rounded-3xl border border-slate-100 shadow-sm p-6 space-y-3">
                            <button type="submit" class="w-full flex items-center justify-center gap-2 bg-navy-900 hover:bg-navy-950 text-white py-3.5 rounded-2xl font-black text-[11px] tracking-widest transition-all shadow-lg shadow-navy-900/20 uppercase">
                                <i class="fas fa-save"></i> Konversi & Simpan
                            </button>
                            <a href="<?php echo e(route('admin.laporan-warga')); ?>" class="w-full flex items-center justify-center gap-2 bg-slate-100 hover:bg-slate-200 text-slate-500 py-3.5 rounded-2xl font-black text-[11px] tracking-widest transition-all uppercase">
    ...
    <script>
        ...
        const lat = <?php echo e($laporan->latitude ?? -3.316694); ?>;
        const lng = <?php echo e($laporan->longitude ?? 114.590111); ?>;
...
<?php /**PATH C:\laragon1\laragon\www\geo-sinfra\resources\views/admin/create-from-laporan.blade.php ENDPATH**/ ?>