    ...
    <main class="flex-1 flex flex-col h-screen overflow-hidden text-left">
        <header class="bg-white/85 backdrop-blur-xl border-b border-slate-100 px-8 py-5 flex justify-between items-center z-40 text-left">
            <div class="flex items-center gap-4 text-left">
                ...
                <div class="text-left">
                    ...
                    <h2 class="text-xl font-black text-navy-900 leading-none">Manajemen Pengguna</h2>
                </div>
            </div>
        ...
        <div class="flex-1 p-8 overflow-y-auto custom-scrollbar">
            ...
            <div class="mb-6 px-6 py-4 bg-red-50 border border-red-100 text-red-700 rounded-2xl flex items-center gap-3">
                ...
                <p class="text-xs font-bold"><?php echo e(session('error')); ?></p>
            </div>
            <?php endif; ?>
            ...
            <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
                <div>
                    ...
                    <p class="text-xs text-slate-400 font-medium text-left font-sans">Kelola hak akses untuk Admin, Surveyor, dan Kabid</p>
                </div>
                
                <div class="flex flex-wrap items-center gap-3 w-full md:w-auto">
                    <form action="<?php echo e(route('admin.users')); ?>" method="GET" class="flex items-center flex-1 md:w-[400px]">
                    ...
                    <a href="<?php echo e(route('admin.users.create')); ?>" class="bg-gold-500 text-white text-xs px-6 py-2.5 rounded-2xl font-bold shadow-lg shadow-gold-500/10 hover:bg-gold-600 hover:shadow-gold-500/20 transition flex items-center gap-2 whitespace-nowrap">
            ...
            <div class="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden mb-10">
                    ...
                    <tbody class="divide-y divide-slate-50">
                        ...
                        <tr class="hover:bg-slate-50/50 transition">
                            <td class="px-8 py-5 text-center">
                                <span class="text-xs font-black text-navy-900"><?php echo e(request('show') == 'all' ? $index + 1 : ($users->currentPage() - 1) * $users->perPage() + $index + 1); ?></span>
                            </td>
                            <td class="px-8 py-5">
                                <div class="flex items-center gap-4">
                                    <div class="w-10 h-10 <?php echo e($user->role == 'admin' ? 'bg-gold-500/10 text-gold-500 border-gold-500/20' : ($user->role == 'kabid' ? 'bg-navy-900/10 text-navy-900 border-navy-900/20' : 'bg-slate-100 text-slate-600 border-slate-200')); ?> rounded-xl flex items-center justify-center font-bold text-xs border">
                                        <?php echo e(substr($user->name, 0, 1)); ?>

                                    </div>
                                    <div>
                                        <p class="text-xs font-black text-navy-900 uppercase leading-none"><?php echo e($user->name); ?></p>
                                        <p class="text-[9px] text-slate-400 font-bold uppercase mt-1 italic">ID: #<?php echo e(str_pad($user->id, 4, '0', STR_PAD_LEFT)); ?></p>
                                    </div>
                                </div>
                            </td>
                            <td class="px-8 py-5 text-[11px] font-medium text-slate-500"><?php echo e($user->email); ?></td>
                            <td class="px-8 py-5">
                                <span class="px-4 py-1.5 rounded-full text-[9px] font-black uppercase tracking-tighter 
                                    <?php echo e($user->role == 'admin' ? 'bg-gold-500/10 text-gold-500 border border-gold-500/20' : ($user->role == 'kabid' ? 'bg-navy-900/10 text-navy-900 border border-navy-900/20' : 'bg-slate-100 text-slate-600 border border-slate-200')); ?>">
                            ...
                            <td class="px-8 py-5">
                                <div class="flex justify-center gap-2">
                                    <?php if($user->role !== 'kabid'): ?>
                                    
                                    <a href="<?php echo e(route('admin.users.edit', $user->id)); ?>" title="Edit User" class="w-8 h-8 flex items-center justify-center bg-gold-500 hover:bg-gold-600 text-white rounded-lg text-xs font-black transition shadow-sm hover:scale-105">
                                        <i class="fas fa-edit"></i>
                                    </a>
                                    
                                    <form action="<?php echo e(route('admin.users.destroy', $user->id)); ?>" method="POST" class="inline-block m-0 p-0" onsubmit="return confirm('PERINGATAN!\n\nApakah Anda yakin ingin menghapus akun milik <?php echo e($user->name); ?>?\nTindakan ini tidak dapat dibatalkan.');">
                                        <?php echo csrf_field(); ?>
                                    ...
                                    <?php else: ?>
                                    <span class="px-3 h-8 bg-slate-100 text-slate-400 text-[10px] font-bold rounded-lg flex items-center justify-center cursor-not-allowed gap-2" title="Akun Kabid dilindungi sistem">
                ...
                <?php if(request('show') != 'all' && isset($users) && $users instanceof \Illuminate\Pagination\LengthAwarePaginator): ?>
...
<?php /**PATH C:\laragon1\laragon\www\geo-sinfra\resources\views/admin/users.blade.php ENDPATH**/ ?>