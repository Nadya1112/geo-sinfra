        ...
        <div class="flex-1 p-8 overflow-y-auto custom-scrollbar">
            ...
            <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
                ...
                <div class="flex flex-row flex-nowrap items-center gap-2 w-full md:w-auto">
                    <form action="<?php echo e(route('admin.activity')); ?>" method="GET" class="flex items-center flex-1 min-w-0 md:w-[400px]">
                        <input type="text" name="search" value="<?php echo e(request('search')); ?>" placeholder="Cari log aktivitas..." class="flex-1 min-w-[80px] pl-4 pr-3 py-2.5 bg-white border border-slate-100 rounded-l-2xl text-[10px] md:text-xs font-semibold focus:outline-none focus:ring-4 focus:ring-gold-500/10 focus:border-gold-500 transition-all shadow-sm">
                    ...
                    <a href="<?php echo e(route('admin.activity.export')); ?>" class="bg-emerald-500 text-white text-xs px-4 md:px-6 py-2.5 rounded-2xl font-bold shadow-lg shadow-emerald-500/10 hover:bg-emerald-600 hover:shadow-emerald-500/20 transition flex items-center justify-center gap-2 whitespace-nowrap shrink-0">
            ...
            <div class="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden mb-10">
                            ...
                            <tr class="hover:bg-slate-50/50 transition-colors group">
                                <td class="px-4 py-3 text-xs text-slate-500 whitespace-nowrap">
                                    <div class="flex flex-col gap-0.5">
                                        <span class="font-bold text-navy-900"><?php echo e($activity->created_at->format('Y-m-d')); ?></span>
                                        <span class="text-[10px] text-slate-400"><i class="fas fa-clock mr-1 text-slate-300"></i> <?php echo e($activity->created_at->format('H:i:s')); ?></span>
                                    </div>
                                </td>
                                <td class="px-4 py-3">
                                    <div>
                                        <p class="font-bold text-navy-900"><?php echo e($activity->user ? $activity->user->name : 'Sistem Otomatis'); ?></p>
                                        <p class="text-[10px] text-slate-400 uppercase tracking-wider"><?php echo e($activity->user ? $activity->user->role : 'System'); ?></p>
                                    </div>
                                </td>
                                <td class="px-4 py-3">
                                    <span class="px-2.5 py-1.5 bg-<?php echo e($badgeColor); ?>-50 text-<?php echo e($badgeColor); ?>-600 text-[10px] font-bold uppercase tracking-wider rounded-lg border border-<?php echo e($badgeColor); ?>-100 inline-flex items-start gap-1.5 shadow-sm max-w-[200px] md:max-w-xs whitespace-normal text-left leading-relaxed">
                                        <i class="fas <?php echo e($icon); ?> shrink-0 mt-0.5"></i> <span><?php echo e($actionName); ?></span>
                                    </span>
                                </td>
                                <td class="px-4 py-3 font-bold text-navy-800 uppercase text-xs"><?php echo e($activity->type); ?> <?php echo $activity->reference_id ? "<span class='text-slate-400 text-xs'>(ID: {$activity->reference_id})</span>" : ""; ?></td>
                            </tr>
                ...
                <?php if($activities instanceof \Illuminate\Pagination\LengthAwarePaginator && $activities->hasPages()): ?>
                ...
                <?php else: ?>
                    <div class="px-8 py-4 border-t border-slate-50 bg-slate-50/30 flex justify-between items-center text-xs text-slate-500 font-bold">
                        <span>Menampilkan total <?php echo e($activities->count()); ?> aktivitas</span>
                    </div>
                <?php endif; ?>
            </div>
...
<?php /**PATH C:\laragon1\laragon\www\geo-sinfra\resources\views\admin\activity.blade.php ENDPATH**/ ?>