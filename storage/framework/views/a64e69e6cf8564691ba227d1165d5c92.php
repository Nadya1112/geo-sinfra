    ...
    <main class="flex-1 flex flex-col h-screen overflow-y-auto custom-scrollbar">
        ...
        <header class="bg-white dark:bg-[#1e1b4b] border-b border-slate-100 dark:border-white/10 px-4 pl-20 md:px-8 py-4 flex justify-between items-center z-40 sticky top-0">
            <div class="flex items-center gap-4 min-w-0">
                <a href="<?php echo e(route('tim_teknis.dashboard')); ?>" class="w-10 h-10 flex items-center justify-center bg-slate-50 dark:bg-[#0f0e2c] text-slate-400 rounded-xl hover:bg-gold-50 hover:text-gold-500 transition-all border border-slate-100 dark:border-white/10 hidden md:flex">
            ...
            <div class="flex items-center gap-3 md:gap-6 flex-shrink-0">
                ...
                <a href="<?php echo e(route('tim_teknis.profile')); ?>" class="flex items-center gap-2 md:gap-3 group">
                    <div class="text-right">
                        <p class="text-sm font-black text-navy-900 dark:text-white leading-none uppercase group-hover:text-gold-500 transition-colors max-w-[200px] truncate hidden md:block"><?php echo e(auth()->user()->name); ?></p>
        ...
        <div class="p-4 md:p-8 space-y-6 md:space-y-8">
            ...
            <div class="bg-white dark:bg-[#1e1b4b] rounded-[2rem] md:rounded-[3rem] shadow-sm border border-slate-100 dark:border-white/10 overflow-hidden mb-10">
                <div class="px-4 md:px-8 py-4 md:py-5 border-b border-slate-50 flex flex-col md:flex-row justify-between items-center bg-white dark:bg-[#1e1b4b] gap-4">
                    ...
                    <div>
                        <form action="<?php echo e(route('tim_teknis.validasi')); ?>" method="GET" class="flex items-center gap-2">
                ...
                <div class="px-4 md:px-8 py-4 bg-white dark:bg-[#1e1b4b] border-b border-slate-100 dark:border-white/10">
                    <form action="<?php echo e(route('tim_teknis.validasi')); ?>" method="GET" class="flex flex-col gap-4">
                        ...
                        <div class="flex flex-wrap gap-2 mb-2">
                            <a href="<?php echo e(route('tim_teknis.validasi', array_merge(request()->query(), ['status' => 'All']))); ?>" class="px-4 py-2 rounded-xl text-xs font-black uppercase tracking-widest transition-all <?php echo e($currentStatus == 'All' ? 'bg-navy-900 text-white shadow-md' : 'bg-white dark:bg-[#1e1b4b] text-slate-400 hover:bg-slate-100 border border-slate-200 dark:border-white/20'); ?>">Semua Antrean</a>
                            <a href="<?php echo e(route('tim_teknis.validasi', array_merge(request()->query(), ['status' => 'Pending']))); ?>" class="px-4 py-2 rounded-xl text-xs font-black uppercase tracking-widest transition-all <?php echo e($currentStatus == 'Pending' ? 'bg-amber-500 text-white shadow-md shadow-amber-500/20' : 'bg-white dark:bg-[#1e1b4b] text-slate-400 hover:bg-slate-100 border border-slate-200 dark:border-white/20'); ?>">Menunggu (Pending)</a>
                            <a href="<?php echo e(route('tim_teknis.validasi', array_merge(request()->query(), ['status' => 'Validated']))); ?>" class="px-4 py-2 rounded-xl text-xs font-black uppercase tracking-widest transition-all <?php echo e($currentStatus == 'Validated' ? 'bg-emerald-500 text-white shadow-md shadow-emerald-500/20' : 'bg-white dark:bg-[#1e1b4b] text-slate-400 hover:bg-slate-100 border border-slate-200 dark:border-white/20'); ?>">Disetujui (Validated)</a>
                            <a href="<?php echo e(route('tim_teknis.validasi', array_merge(request()->query(), ['status' => 'Rejected']))); ?>" class="px-4 py-2 rounded-xl text-xs font-black uppercase tracking-widest transition-all <?php echo e($currentStatus == 'Rejected' ? 'bg-rose-500 text-white shadow-md shadow-rose-500/20' : 'bg-white dark:bg-[#1e1b4b] text-slate-400 hover:bg-slate-100 border border-slate-200 dark:border-white/20'); ?>">Ditolak / Perbaikan</a>
                        </div>
                        ...
                        <div class="flex flex-wrap md:flex-nowrap gap-4 items-end bg-slate-50 dark:bg-[#0f0e2c]/50 p-4 rounded-2xl border border-slate-100 dark:border-white/10">
                            ...
                            <div class="w-full md:flex-1">
                                ...
                                <input type="date" name="start_date" value="<?php echo e(request('start_date')); ?>" class="w-full bg-white dark:bg-[#1e1b4b] border border-slate-200 dark:border-white/20 rounded-xl px-4 py-2.5 text-xs font-bold text-navy-900 dark:text-white focus:outline-none focus:border-gold-500 transition-all shadow-sm">
                            </div>
                            <div class="w-full md:flex-1">
                                <label class="text-xs font-black text-slate-400 uppercase tracking-widest block mb-2">Sampai Tanggal</label>
                                <input type="date" name="end_date" value="<?php echo e(request('end_date')); ?>" class="w-full bg-white dark:bg-[#1e1b4b] border border-slate-200 dark:border-white/20 rounded-xl px-4 py-2.5 text-xs font-bold text-navy-900 dark:text-white focus:outline-none focus:border-gold-500 transition-all shadow-sm">
                            </div>
                            <div class="w-full md:flex-[0.5] flex gap-2 justify-end">
                                ...
                                <a href="<?php echo e(route('tim_teknis.validasi', ['status' => $currentStatus])); ?>" class="px-4 py-2.5 bg-white dark:bg-[#1e1b4b] text-slate-400 rounded-xl text-xs font-black uppercase tracking-widest hover:bg-slate-100 hover:text-slate-600 transition-all border border-slate-200 dark:border-white/20 shadow-sm flex items-center justify-center">
                            ...
                            <tr class="hover:bg-slate-50 dark:bg-[#0f0e2c]/50 transition-colors group">
                                ...
                                <td class="px-6 py-5 min-w-[280px]">
                                    <div class="flex items-center gap-4">
                                        ...
                                        <div>
                                            <h4 class="text-[13px] font-black text-navy-900 dark:text-white leading-tight mb-0.5"><?php echo e($item->nama_objek); ?></h4>
                                            <p class="text-xs font-bold text-slate-400 uppercase tracking-widest"><?php echo e($item->jenis); ?></p>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-5 min-w-[150px]">
                                    <p class="text-xs font-bold text-navy-900 dark:text-white mb-0.5"><?php echo e($item->kelurahan->nama_kelurahan ?? '-'); ?></p>
                                    <p class="text-xs text-slate-400 font-bold uppercase tracking-widest"><?php echo e($item->kelurahan->kecamatan->nama_kecamatan ?? '-'); ?></p>
                                </td>
                                <td class="px-6 py-5 min-w-[150px]">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <p class="text-xs font-black text-navy-900 dark:text-white"><?php echo e($item->user->name ?? 'Anonim'); ?></p>
                                    </div>
                                </td>
                                <td class="px-6 py-5 min-w-[150px]">
                                    <div class="flex flex-col gap-2 items-start">
                                        ...
                                        ?>
                                        <span class="px-2.5 py-1 rounded-md border text-xs font-black uppercase tracking-widest whitespace-nowrap <?php echo e($aiClass); ?>">
                                ...
                                <td class="px-6 py-5 min-w-[140px]">
                                    <div class="flex flex-col gap-2">
                                        ...
                                        ?>
                                        <span class="px-3 py-1.5 rounded-xl text-xs font-black uppercase tracking-widest border flex items-center gap-2 w-fit <?php echo e($statusClass); ?>">
                                            <i class="fas <?php echo e($statusIcon); ?> text-xs"></i>
                                ...
                                <td class="px-6 py-5 min-w-[260px]">
                                    <div class="flex items-center justify-center gap-2">
                                        ...
                                        <a href="<?php echo e(route('tim_teknis.infrastruktur.show', $item->id_infrastruktur)); ?>" class="flex items-center justify-center gap-2 px-3 py-2.5 bg-navy-50 text-navy-900 dark:text-white rounded-xl hover:bg-gold-500 hover:text-white transition-all border border-navy-100 shadow-sm group" title="Lihat Detail">
                                        ...
                                        <?php if($item->status_validasi == 'Pending'): ?>
                                            <form action="<?php echo e(route('tim_teknis.validasi.proses', $item->id_infrastruktur)); ?>" method="POST" class="flex-1" onsubmit="return promptCatatan(event, this, 'Validated')">
                                                <?php echo csrf_field(); ?>
                                            ...
                                            <form action="<?php echo e(route('tim_teknis.validasi.proses', $item->id_infrastruktur)); ?>" method="POST" class="flex-1" onsubmit="return promptCatatan(event, this, 'Rejected')">
                                                <?php echo csrf_field(); ?>
...
<?php /**PATH C:\laragon1\laragon\www\geo-sinfra\resources\views\tim_teknis\validasi.blade.php ENDPATH**/ ?>